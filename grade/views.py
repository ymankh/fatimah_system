...
def only_staff_message(
    request, message="سجل الدخول بحساب مصرح لتتمكن من الوصل لهذه الصفحة!!"
):
...
def login(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.add_message(
                request, messages.WARNING, "معلومات الدخول غير صحيحة، حاول مرة اخرى"
            )
...
def absence_table(request, grade_id, section_id, group_id):
    ...
    if group_id == 0:
        students = (
            ...
            .filter(grade=grade, section=section)
            .order_by("full_name")
        )
    else:
        students = (
    ...
    return render(
        request,
        "students_table.html",
        {"grade_id": grade_id, "section_id": section_id, "students": students, "grade":grade, "section":section},
    )
...
def chose_grade(request):
    ...
    page_options = (
        ["students_table", "الغيياب"],
        ["participation_table", "المشاركة في الحصة"],
        ["chose_skill", "قياس مهارات"],
        ["chose_subject_for_skill_table", "جدول المهارات"],
    )
    return render(
        request,
        "chose_grade.html",
